...
def gbClassificationWithOriginalData(path_data_array, outputColumnIndex):
    ...
    model = GradientBoostingClassifier(loss='deviance', #deviance or explonential
                               ...
                               min_samples_leaf=100, # minimum no of observarions allowed in a leaf
                               max_depth=5, # maximum depth of the tree
                               n_estimators=1500 # how many trees to build
                              )
...
def gbClassificationWithRepartitionedData(path_data_array, path_group_data, outputColumnIndex):
    m_start = get_process_memory()
    file_original_data = open(path_data_array, "rb")
    original_data_2d = np.load(file_original_data)
    num_rows, num_cols, num_attrs = original_data_2d.shape
